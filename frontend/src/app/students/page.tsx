    ...
    <DashboardLayout>
      <div className="flex flex-col gap-8">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="bg-primary text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-primary/30 hover:shadow-primary/40 transition-all"
          >
            <UserPlus size={20} />
            Add New Student
          </button>
        ...
        <div className="glass p-4 rounded-2xl border flex items-center gap-4">
          ...
          <select className="bg-muted/50 border-none rounded-lg px-4 py-2 text-sm outline-none font-medium">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass w-full max-w-2xl rounded-3xl border shadow-2xl overflow-hidden animate-in">
            <div className="p-6 border-b flex items-center justify-between">
              <h2 className="text-2xl font-bold tracking-tight">Register Student</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-accent rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold">Face Registration (Min 3 photos)</label>
                <div className="border-2 border-dashed border-primary/20 rounded-2xl p-6 text-center">
                  <div className="flex flex-wrap gap-3 mb-4 justify-center">
                    {previews.map((url, i) => (
                      <div key={i} className="w-20 h-20 rounded-lg overflow-hidden border relative group">
                    ...
                    {previews.length < 5 && (
                      <button 
                        ...
                        className="w-20 h-20 rounded-lg border-2 border-dashed border-muted-foreground/20 flex flex-col items-center justify-center text-muted-foreground hover:bg-muted/50 transition-all"
                      >
                        <Camera size={20} />
                        <span className="text-[10px] mt-1 uppercase font-bold">Add</span>
                      </button>
                    )}
                  </div>
                  <input 
                    ...
                    accept="image/*"
                  />
                  <p className="text-xs text-muted-foreground">Upload clear portrait photos of the student</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 py-4 bg-muted text-foreground rounded-xl font-bold hover:bg-muted/80 transition-all"
                >
                  Cancel
                </button>
                <button 
